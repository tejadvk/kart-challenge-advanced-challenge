...
A React frontend is available in the [frontend](./frontend/) directory. It integrates with either the demo API or the local backend.
...
Run the entire stack (PostgreSQL, Redis, backend API, frontend) with:
...
The frontend is built with `VITE_API_URL=http://localhost:8081` for Docker.
